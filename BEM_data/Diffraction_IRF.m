function hydro = Diffraction_IRF(hydro,t_end,n_t,n_w,w_min,w_max)

% Calculates the normalized diffraction impulse response function.
% 
% hydro = Diffraction_IRF(hydro, t_end, n_t, n_w, w_min, w_max)
...
for k = 1:length(t)
    for i = 1:sum(hydro.dof)
        for j = 1:hydro.Nh
            sc_re = interp1(hydro.w,squeeze(hydro.sc_re(i,j,:)),w);
            sc_im = interp1(hydro.w,squeeze(hydro.sc_im(i,j,:)),w);
            hydro.sc_K(i,j,k) = (1/pi)*trapz(w,sc_re.*cos(w*t(k))-sc_im.*sin(w*t(k)));
            n = n+1;
        end
    end
    waitbar(n/N)
